...
def main(
    language_file, encoding, errors
):  # main function; 'encoding' - by what rules, errors - what to do in case of an error
    line = language_file.readline()  # read file line by line

    if line:  # checks if there is something in the string
        print_line(
            line, encoding, errors
        )  # if the string is not empty, prints the string and ...
...
def print_line(line, encoding, errors):  # function encodes the string read on line 6
    next_lang = (
        line.strip()
    )  # removes spaces at the end and at the beginning of a line, but not inside the line itself
    raw_bytes = next_lang.encode(
        encoding, errors=errors
    )  # string is converted in bytes
    cooked_string = raw_bytes.decode(
        encoding, errors=errors
    )  # bytes are converted in string

    print(
        raw_bytes, "<===>", cooked_string
    )  # display the result on the screen, on the left are bytes in utf-8 encoding, on the right is text
