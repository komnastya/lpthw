...
from urllib.request import urlopen ##urllib.request is a Python module for openning URLs
...
def convert(snippet, phrase):
    ...
    class_names = [tmr.capitalize() for tmr in
    ...
    for sentence in snippet, phrase: #key, value
        ...
        for word in class_names:
            result = result.replace("%%%", word, 1) #replaces the first marker from class_name list


        for word in other_names:
            result = result.replace("***", word, 1) #replaces the first marker from other_name list


        for word in param_names:
            result = result.replace("@@@", word, 1) #replaces the first marker from param_names list
...
try:
    while True:
        snippets = list(PHRASES.keys()) #'PHRASES' dictionary keys forms 'snippets' list
        random.shuffle(snippets) #shuffles the 'snippets' list (reorganizes the order of the list items)

        for snippet in snippets:
            #snippet is one of the list elements which is key of 'PHRASES' dictionary
            phrase = PHRASES[snippet] #phrase is value of 'PHRASES' dictionary (e.g. Make a class named %%% that is-a %%%) found by key from 'snippets' list
            question, answer = convert(snippet, phrase)
            if PHRASE_FIRST: #if PHRASE_FIRST = True
                question, answer = answer, question #... answer and question are swapped
