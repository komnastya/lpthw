def break_words(stuff):  # function #1
    ...
    words = stuff.split(
        " "
    )  # separates words in a sentence and displays them in an array in quotes, separated by commas
    return words
...
def print_first_word(words):  # function #3
    ...
    word = words.pop(
        0
    )  # cuts out the first word, but if you call the method again, the second word will be first
    print(word)
...
def print_last_word(words):  # function #4
    ...
    word = words.pop(
        -1
    )  # selects the last word, but if you call the method again, then the "last" will be the penultimate word
    print(word)
...
def sort_sentence(sentence):  # function #5
    ...
    return sort_words(
        words
    )  # function #2 sorts the words received by function #1 inside function #5, thus function #5 returns the result of function #2
