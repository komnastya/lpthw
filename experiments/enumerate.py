...
def even_items (list):
  ...
  for index, value in enumerate(list, start=1):
    if not index % 2: #check whether the remainder of dividing index by 2 is zero. If it is, then you append the item to values.
      output.append(value)
  return output
...
enum_instance = enumerate(values) #pass values to enumerate() and assign the return value to enum_instance
print (enum_instance) # print enum_instance, you can see that it’s an instance of enumerate() with a particular memory address
print (next(enum_instance)) #built-in next() to get the next value from enum_instance. The first value that enum_instance returns is a tuple with the count 0 and the first element from values, which is "a".
print (next(enum_instance)) #calling next() again on enum_instance yields another tuple, this time with the count 1 and the second element from values, "b"
